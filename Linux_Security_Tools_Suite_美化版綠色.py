...
class ToolPageBase(QtWidgets.QWidget):
    ...
    def _build_ui(self):
        ...
        self.desc.setStyleSheet("padding:8px; background:transparent; color:#2b2b2b;")
        ...
        self.output.setStyleSheet("background:#ffffff; border:1px solid #e9e9e9;")
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        # left nav - flat
        self.nav = QtWidgets.QListWidget(); self.nav.setFixedWidth(200); self.nav.setSpacing(6)
        ...
        for t in tools:
            ...
            self.nav.addItem(it)
        # flat styling - no extra inner frames
        self.nav.setStyleSheet("""
            QListWidget { background: #f7f8f7; border: none; padding:6px; }
            QListWidget::item { padding:10px; border-radius:2px; }
            QListWidget::item:selected { background:#2ea84a; color:white; font-weight:700; }
        """)
    ...
    def _apply_style(self):
        try:
            ...
            pal = QtGui.QPalette()
            pal.setColor(QtGui.QPalette.Window, QtGui.QColor(249,250,249))
            ...
            pal.setColor(QtGui.QPalette.Text, QtGui.QColor(28,28,28))
            pal.setColor(QtGui.QPalette.Button, QtGui.QColor(245,245,245))
            pal.setColor(QtGui.QPalette.ButtonText, QtGui.QColor(28,28,28))
            pal.setColor(QtGui.QPalette.Highlight, QtGui.QColor(46,168,74))
            ...
            QPushButton { padding:6px 10px; border:1px solid #d8d8d8; border-radius:2px; background:#ffffff; }
            QPushButton:hover { background:#f3fff3; }
            QLineEdit, QComboBox, QPlainTextEdit { padding:6px; border:1px solid #e3e3e3; border-radius:2px; }
            QProgressBar { border:1px solid #d8d8d8; border-radius:2px; height:12px; background:#f5f5f5; }
            QProgressBar::chunk { background: qlineargradient(spread:pad, x1:0, y1:0, x2:1, y2:0, stop:0 #8bff8b, stop:1 #2eb82e); }
            QListWidget { background:#fbfbfb }
            """
            QtWidgets.QApplication.setStyleSheet(qss)
        except Exception:
            pass
